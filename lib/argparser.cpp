...
namespace nargparse {
    ...
    void FreeParser(ArgumentParser& parser) {
        ...
        for (int i = 0; i < parser.arguments_count; i++) {
            if (parser.arguments[i].type == ARG_INT && parser.arguments[i].repeated_count > 1) {
                delete[] parser.arguments[i].repeated_arr.int_arr;
            } else if (parser.arguments[i].type == ARG_FLOAT && parser.arguments[i].repeated_count > 1) {
                delete[] parser.arguments[i].repeated_arr.float_arr;
            } else if (parser.arguments[i].type == ARG_CHAR && parser.arguments[i].repeated_count > 1) {
    ...
    void AddFlag(ArgumentParser& parser, char* long_flag, bool* flag, char* flag_name, bool new_flag_val) {
        *flag = new_flag_val;
        parser.flags_count++;
    ...
    void AddFlag(ArgumentParser& parser, char* short_flag, char* long_flag, bool* flag, char* flag_name, bool new_flag_val) {
        *flag = new_flag_val;
        parser.flags_count++;
        int flags_count = parser.flags_count;
    ...
    void AddArgument(ArgumentParser& parser,  char* short_arg, char* long_arg, int* arg_value, char* arg_name, const uint8_t kNargs, bool (*CheckFunc)(const int&), char* condition) {
        Argument new_arg; 
    ...
    void AddArgument(ArgumentParser& parser,  char* short_arg, char* long_arg, float* arg_value, char* arg_name, const uint8_t kNargs, bool (*CheckFunc)(const float&), char* condition) {
        Argument new_arg; 
        new_arg.arg_name = arg_name;
    ...
    void AddArgument(ArgumentParser& parser, int* arg_value, char* arg_name, const uint8_t kNargs, bool (*CheckFunc)(const int&), char* condition) {
        Argument new_arg; 
        new_arg.arg_name = arg_name;
        new_arg.arg_value.int_value = arg_value;
    ...
    void AddArgument(ArgumentParser& parser, float* arg_value, char* arg_name, const uint8_t kNargs, bool (*CheckFunc)(const float&), char* condition) {
        Argument new_arg; 
        new_arg.arg_name = arg_name;
        new_arg.arg_value.float_value = arg_value;
        new_arg.arg_short = nullptr;
    ...
    void AddArgument(ArgumentParser& parser,  char* short_arg, char* long_arg, void* arg_value, char* arg_name, const uint8_t kNargs, bool (*CheckFunc)(const char* const&), char* condition) {
        char* buffer = (char*)(arg_value);
    ...
    void AddArgument(ArgumentParser& parser, void* arg_value, char* arg_name, const uint8_t kNargs, bool (*CheckFunc)(const char* const&), char* condition) {
        char* buffer = (char*)(arg_value);
    ...
    bool ArgToParser(ArgumentParser& parser, Argument* arg, char* curr) {
        ...
        if (len >= parser.kMaxArgLen) {
            return false;
        }

        if (arg->repeated_count == 0) {
            std::strcpy(arg->arg_value.char_value, curr);
            arg->repeated_count = 1;
        } else {
    ...
    bool ArgProccess(ArgumentParser& parser, int argc, const char** argv, char** arg_name, char** flag_name, const char* curr_arg, int& i) {
        int arg_index = -1;
        int flag_index = -1;
        ...
        if (arg_key[1] == '-') {
            ...
            for (int j = 0; j < parser.arguments_count; ++j) {
                if (parser.arguments[j].arg_long != nullptr && strcmp(arg_key, parser.arguments[j].arg_long) == 0) {
                    if (*arg_name != nullptr || *flag_name != nullptr) {
                        if (equal_pos != nullptr) delete[] arg_key;
                        return false;
                    }
            ...
            for (int j = 0; j < parser.arguments_count; ++j) {
                if (parser.arguments[j].arg_short != nullptr && strcmp(curr_arg, parser.arguments[j].arg_short) == 0) {
                    if (*arg_name != nullptr || *flag_name != nullptr) {
                        if (equal_pos != nullptr) delete[] arg_key;
                        return false;
                    }
            ...
            if (arg_index == -1) {
                for (int j = 0; j < parser.flags_count; ++j) {
                    if (parser.flags[j].flag_short != nullptr && strcmp(curr_arg, parser.flags[j].flag_short) == 0) {
                        if (*flag_name != nullptr || *arg_name != nullptr) {
                            if (equal_pos != nullptr) delete[] arg_key;
                            return false;
                        }
        ...
        if (arg_index != -1) {
            ...
            if (arg->type == ARG_INT) {
                ...
                int curr = std::atoi(str_value_arg);
                ...
                float curr = strtof(str_value_arg, nullptr);
                if (!ArgToParser(parser, arg, curr)) {
                    if (equal_pos != nullptr) delete[] arg_key;
                    return false;
                }
            } else if (arg->type == ARG_CHAR) {
                char* curr = (char*)str_value_arg;
                if (!ArgToParser(parser, arg, curr)) {
                    if (equal_pos != nullptr) delete[] arg_key;
                    return false;
                }
    ...
    int GetRepeatedCount(ArgumentParser parser, char* arg_name) {
        for (int i = 0; i < parser.arguments_count; i++) {
            if (strcmp(arg_name, parser.arguments[i].arg_name) == 0) {
    ...
    bool GetRepeated(ArgumentParser& parser, char* arg_name, int n, int* value) {
        for (int i = 0; i < parser.arguments_count; i++) {
            if (strcmp(arg_name, parser.arguments[i].arg_name) == 0 && parser.arguments[i].type == ARG_INT) {
    ...
    bool GetRepeated(ArgumentParser& parser, char* arg_name, int n, float* value) {
        for (int i = 0; i < parser.arguments_count; i++) {
            if (strcmp(arg_name, parser.arguments[i].arg_name) == 0 && parser.arguments[i].type == ARG_FLOAT) {
                if (n < 0 || n >= parser.arguments[i].repeated_count) {
                    return false;
                }
                if (n == 0) {
    ...
    bool GetRepeated(ArgumentParser& parser, char* arg_name, int n, char** value) {
        for (int i = 0; i < parser.arguments_count; i++) {
            if (strcmp(arg_name, parser.arguments[i].arg_name) == 0 && parser.arguments[i].type == ARG_CHAR) {
                if (n < 0 || n >= parser.arguments[i].repeated_count) {
                    return false;
                }
                if (n == 0) {
    ...
    void AddHelp(ArgumentParser& parser) {
        ...
        AddFlag(parser, "--help", &help_requested, "Show help message", false);
    }
    ...
    bool Parse(ArgumentParser& parser, int argc, const char** argv) {
        ...
        for (int t = 0; t < targets_count; t++) {
            ...
            for (int n = 0; n < take_count; n++) {
                ...
                if (arg->type == ARG_INT) {
                    ...
                    if (!ArgToParser(parser, arg, (char*)str_val)) {
                        delete[] positional_args;
                        return false;
                    }
                }
        ...
        for (int i = 0; i < parser.arguments_count; i++) {
            Argument& arg = parser.arguments[i];
